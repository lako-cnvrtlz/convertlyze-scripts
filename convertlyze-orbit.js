...
(function () {
  ...
  // ── Sizes — edit these to scale the whole graphic ─────────────────────────
  var W   = 680;   // total SVG width
  var H   = 600;   // total SVG height
  var R   = 218;   // orbit radius
  var NW  = 92;    // node width
  var NH  = 78;    // node height
  var MKW = 130;   // mockup width
  var MKH = 178;   // mockup height
  // ─────────────────────────────────────────────────────────────────────────
  ...
  function init() {
    ...
    style.textContent =
      "#cvly-orbit-wrap{width:100%;max-width:" + W + "px;margin:0 auto;position:relative;user-select:none;font-family:system-ui,-apple-system,sans-serif;}" +
      "#cvly-orbit-wrap svg{display:block;width:100%;height:auto;}" +
      ".cvly-node{position:absolute;border-radius:16px;background:#161b27;border:1.5px solid rgba(79,209,197,0.22);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:5px;cursor:pointer;transition:all 0.3s ease;box-sizing:border-box;padding:8px 6px;}" +
      ...
      "#cvly-panel{position:absolute;left:50%;transform:translateX(-50%);width:300px;bottom:8px;" +
        "padding:14px 16px;border-radius:14px;background:#1e2535;" +
        "border:1px solid rgba(79,209,197,0.2);box-shadow:0 8px 32px rgba(0,0,0,0.5);transition:border-color .3s;}" +
      ...
      "#cvly-panel .pf{font-size:13px;font-weight:600;color:#e2e8f0;margin-bottom:6px;line-height:1.4;}" +
      "#cvly-panel .pt{font-size:12px;color:#94a3b8;line-height:1.5;}" +
      "#cvly-panel .hint{font-size:12px;color:#64748b;text-align:center;display:block;}" +
      ".cvly-cc{position:absolute;left:50%;top:50%;transform:translate(-50%,-60%);z-index:20;}" +
      ".cvly-frame{padding:12px;border-radius:18px;background:linear-gradient(135deg,#161b27,#1e2535);" +
        "border:1.5px solid rgba(79,209,197,0.22);box-shadow:0 0 28px rgba(79,209,197,0.18);transition:border-color .4s,box-shadow .4s;}" +
      ".cvly-mock{border-radius:10px;overflow:hidden;border:1px solid rgba(79,209,197,0.15);background:#0a0f1a;}" +
      ".cvly-bar{height:14px;background:#1e2535;display:flex;align-items:center;padding-left:6px;gap:4px;}" +
      ".cvly-dot{width:5px;height:5px;border-radius:50%;opacity:.7;}" +
      ".cvly-body{padding:5px 5px 0;}" +
      ".cvly-s{border-radius:5px;margin-bottom:4px;border:1px solid transparent;transition:all .35s ease;overflow:hidden;}" +
      ".cvly-l{border-radius:2px;margin-bottom:3px;transition:background .35s;}";
    document.head.appendChild(style);

    // ── Mockup rows (sizes scale with MKW/MKH) ───────────────────────────────
